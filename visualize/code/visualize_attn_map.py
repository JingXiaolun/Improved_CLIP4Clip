# author: jingxiaolun
# date: 2023.07.04
# description: attn map in transformer encoder visualization
...
get_local.activate()
from modules.module_cross import Attention
...
def grid_show(to_shows, cols):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            axs[i, j].set_yticks([])
            axs[i, j].set_xticks([])
    plt.savefig(f'../visualization/attn_map_of_multiple_heads.png')
...
def visualize_head(att_map):
    ...
    plt.savefig(f'../visualization/attn_map_of_head_0.png')
    
# Attention map of multiple heads correponding to a specified layer
...
if __name__ == '__main__':
    ################################################ obtaining attention maps from cache ###############################################################################################
    #cache = get_local.cache
    #attention_maps = cache['Attention.forward']
    #print(attention_maps)

    # step1: load attn_maps from attn.pt (shape of [batch, heads, L, L])
    att_maps = torch.load('../attention/attn.pt', map_location='cpu')

    # step2: attn_maps visualization
    visualize_head(att_maps[0][0])
    visualize_heads(att_maps[0], cols=3)
